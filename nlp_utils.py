...
def clean_data(df, dirty_column, clean_column, nof_considated_chars=3, remove_stop_words=False, stop_words=None):
    ...
    df[clean_column] = df[clean_column].map(lambda x: consolidate_k_chars(x, nof_considated_chars))
